...
@RabbitListener(queuesToDeclare = @Queue(RabbitConst.EMAIL_QUEUE_NAME))
public class RabbitEmailConsumer {

    @RabbitHandler
    public void process(MailDTO mailDTO, Message message, Channel channel) {
        // 处理发邮件业务
        log.info("接收到队列消息，开始发送邮件{}", mailDTO);
